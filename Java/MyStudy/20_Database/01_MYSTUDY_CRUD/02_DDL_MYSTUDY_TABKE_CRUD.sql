...
-----
-- DML : INSERT INTO~(삽입), UPDATE, DELETE(삭제), SELECT(선택)
-- 테이블 컬럼명을 명시적으로 지정해서 데이터 입력
INSERT INTO MEMBER (ID, NAME, PASSWORD)
VALUES ('HONG', '홍길동', '1234'); -- 작은따옴표'가 문자열을 의미

-- DCL : ROLLBACK, COMMIT

COMMIT; -- : 최종 문장에 대해 임시 처리된 작업 최종 적용
ROLLBACK; -- 최종 문장에 대해 임시 처리된 작업 취소 처리 (되돌리기)
------------------------
...
VALUES('HONF3', '홍길동3');
------------------------------
SELECT * FROM MEMBER; -- 테이블에 있는 모든 데이커 + 모든 컬럼
...
INSERT INTO MEMBER
VALUES ('HONG3', '홍길동3', '1234', '010-1111-1111', '서울'); -- 생략하면 모든 데이터를 입력
COMMIT;

-- 20BYTE 넘어가면 ORA-12899: value too large for column
-- 입력 위치 바뀌면 잘못된 데이터가 들어갈 수 있음
INSERT INTO MEMBER
...
-- SQL 작성 형식2 - SQL 내용 시작 위치에 일치시키기
UPDATE MEMBER
   SET PHONE = '010-1111-1234',
       ADDRESS = '서울시'
 WHERE ID = 'HONG4'
...
INSERT INTO MEMBER
VALUES ('HONG', '홍길동', '1111', '010-2222-3423', '서울');
COMMIT;
SELECT * FROM MEMBER;
