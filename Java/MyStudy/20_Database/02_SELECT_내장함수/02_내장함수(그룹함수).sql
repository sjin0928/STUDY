...
SELECT COUNT(CUSTID) FROM CUSTOMER; -- 5건 (PRIMARY KEY, PK)
SELECT COUNT(PHONE) FROM CUSTOMER; -- 4건 (NULL 값 X)
...
-- 출판된 책 개수(권수), 합계금액, 평균금액, 가장 비싼 책 금액, 가장 싼 책 금액
SELECT * FROM BOOK;
...
;
--5. 박지성이 구매한 도서의 수 
SELECT '박지성' AS NAME, COUNT(CUSTID)
FROM ORDERS
WHERE CUSTID = 1
;
