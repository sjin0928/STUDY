...
-- 인덱스 생성 : BOOK 테이블의 출판사(PUBLISHER)컬럼에 IX_BOOK_PUB인덱스 만들기
CREATE INDEX IX_BOOK_PUB ON BOOK (PUBLISHER);
SELECT * FROM BOOK ORDER BY PUBLISHER; -- 인덱스했을때의 가나다 순으로 데이터 정렬
SELECT * FROM BOOK; -- 인덱스 안했을때 정렬
-- PUBLISHER를 인덱스로 만들면 PUBLISHER의 각 이름으로 데이터가 쪼개져 있음
SELECT * FROM BOOK WHERE PUBLISHER = '굿 스포츠';
SELECT * FROM BOOK WHERE BOOKNAME = '축구의 이해';

-- 인덱스 생성 : 두개의 2개의 컬럼 기준
...
-- 인덱스 사용 안됨
SELECT * FROM BOOK WHERE PRICE >= 7000 AND BOOKNAME = '축구의 이해';
-- PRICE 일반 검색, BOOKNAME 일반 검색임
...
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%미디어'; -- 인덱스 사용못함
SELECT * FROM BOOK WHERE PUBLISHER LIKE '%대한%'; -- 인덱스 사용못함
